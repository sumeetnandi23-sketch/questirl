...
export default function QuestIRL() {
  ...
  useEffect(() => {
    // Allow reset via ?reset=true in URL
    ...
    const saved = db.load();
    // Check if save has the new dashboard fields — if not, it's an old save
    if (saved && saved.classId && saved.maxHp) {
      setPlayer(saved);
      setScreen("dashboard");
    } else {
      // Old or broken save — clear it and start fresh
      if (saved) localStorage.removeItem(SAVE_KEY);
      setScreen("title");
    }
  }, []);
